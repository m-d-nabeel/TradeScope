...
export function PortfolioSummary<T extends PortfolioResponse>({
  ...
  const latestProfitLoss = parseFloat(data.profit_loss[data.profit_loss.length - 1]);
  const latestProfitLossPct = parseFloat(data.profit_loss_pct[data.profit_loss_pct.length - 1]);
  ...
  return (
    <div className="rounded-lg bg-white p-6 shadow-lg">
      ...
      >
        {latestProfitLoss >= 0 ? <ChevronUp className="mr-1" /> : <ChevronDown className="mr-1" />}
        <span>${Math.abs(latestProfitLoss).toLocaleString()}</span>
        <span className="ml-2 text-sm text-gray-600">({latestProfitLossPct.toFixed(2)}%)</span>
      </motion.div>
    </div>
  );
}
